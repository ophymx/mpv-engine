...
//! WGL context with `WGL_NV_DX_interop2` — Windows + `export` feature
//! only. Like the headless suite, missing tooling degrades to a skip: no
//! ffmpeg, no libmpv, or no GPU access (no OpenGL ICD, no interop
//! support, no window station on a bare CI runner) all print and return
//! instead of failing.
#![cfg(all(feature = "export", target_os = "windows"))]
...
/// `DXGI_FORMAT_B8G8R8A8_UNORM` — the only format this backend exports.
...
fn outstanding_frame_survives_detach() {
    ...
    // Detach (joins the render thread, closes the interop device and
    // destroys the GL context) with the frame still in hand: the shared
    // texture must stay alive and readable — `copy_pixels` runs on the
    // D3D11 device the frame keeps its own reference on, with no GL and
    // no interop involved — and nothing may deadlock or crash, including
    // the frame's return to the torn-down pool afterwards and dropping
    // the engine last.
    engine.detach_render();
    let px = frame.copy_pixels();
    assert_eq!(px.len(), 64 * 64 * 4);
